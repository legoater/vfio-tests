...
# Format a byte count with an appropriate binary unit, so small BARs
# still print a meaningful value instead of rounding to "0.0GiB".
human_size() {
    awk -v b="$1" 'BEGIN {
...
if [ "$fail" -eq 0 ]; then
    ...
    echo "result: FAIL ($fail configuration(s) exceeded threshold): ${fail_mode%??}"
fi
